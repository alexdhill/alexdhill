...
function calculateCommits(gql, years) {
    return __awaiter(this, void 0, void 0, function* () {
        let q = `{viewer{`;
...
function getUserInfo(gql) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const { viewer, rateLimit } = yield gql(query);
        return {
...
function getLanguages(repos) {
    ...
    const getPercent = (size) => ((size / total) * 100);
...
function replaceLanguages(input, repos) {
    ...
    for (const match of input.matchAll(lang_start)) {
        ...
        const rep = getLanguages(repos)
            ...
            let res = str;
            let bar = "";
            for (var i = 0; i < (lang.percent / 4); i++)
                bar += "█";
            bar += ((lang.percent / 4) / 2) ? "▓" : "▒";
            for (var i = 0; i < (25 - (lang.percent / 4)); i++)
                bar += "░";
